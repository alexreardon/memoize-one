import { memoizeOneCount as memoize, EqualityFn } from '../src/memoize-one';
...
describe('standard behaviour - baseline', () => {
  ...
  it('should not execute the memoized function if the number of calls is exceeded', () => {
    memoizedAdd = memoize(add, 3);
    memoizedAdd(1, 2);
    memoizedAdd(1, 3);
    memoizedAdd(1,4);
...
describe('respecting "this" context', () => {
  describe('original function', () => {
    ...
    it('should respect hard bindings', () => {
      ...
      const memoized = memoize(getA.bind(temp), 1);
    ...
    it('should respect fat arrow bindings', () => {
      ...
      const memoized = memoize(bound, 1);
  ...
  describe('memoized function', () => {
    it('should respect new bindings', () => {
      const memoizedGetA = memoize(getA, 2);
    ...
    it('should respect implicit bindings', () => {
      const getAMemoized = memoize(getA, 1);
      const temp = {
        a: 5,
        getAMemoized,
      };
    ...
    it('should respect hard bindings', () => {
      ...
      const getAMemoized = memoize(getA, 1).bind(temp);
    ...
    it('should memoize hard bound memoized functions', () => {
      ...
      const getAMemoized = memoize(spy, 1).bind(temp);
    ...
    it('should respect fat arrow bindings', () => {
      ...
      const memoizedGetA = memoize(getA, 1);
      function foo() {
        // return an arrow function
        return (): number => {
          // `this` here is lexically adopted from `foo()`
...
describe('context change', () => {
  ...
  it('should break the memoization cache if the execution context changes and respect number of calls limit', () => {
    const memoized = memoize(getA, 2);
    const temp1 = {
      a: 20,
      getMemoizedA: memoized,
    ...
    expect(temp1.getMemoizedA()).toBe(20);
    expect(temp2.getMemoizedA()).toBe(30);
    expect(temp3.getMemoizedA()).toBe(30);
  });
});
...
describe('skip equality check', () => {
  it('should not run any equality checks if the "this" context changes', () => {
    ...
    const memoized = memoize(getA, 2, isEqual);
    const obj1: HasA = {
      a: 10,
    };
    const obj2: HasA = {
  ...
  it('should run a custom equality check if the arguments length changes', () => {
    const mock = jest.fn();
    const isEqual = jest.fn().mockReturnValue(true);
    const memoized = memoize(mock, 1, isEqual);
...
describe('custom equality function', () => {
  ...
  it('should call the equality function with the newArgs, lastArgs and lastValue', () => {
    ...
    memoizedAdd = memoize(add, 2, equalityStub);
  ...
  it('should have a nice isDeepEqual consumption story', () => {
    ...
    const memoized = memoize(addAges, 2, isDeepEqual);
  ...
  it('should return the previous value without executing the result fn if the equality fn returns true', () => {
    (equalityStub as jest.Mock).mockReturnValue(true);
    memoizedAdd = memoize(add, 1, equalityStub);
...
describe('throwing', () => {
  ...
  it('should throw when the memoized function throws', () => {
    ...
    const memoized = memoize(willThrow, 1);
  ...
  it('should not break the memoization cache of a successful call', () => {
    ...
    const memoized = memoize(canThrow, 2);
    let firstError;
    let secondError;
  ...
  it('should throw regardless of the type of the thrown value', () => {
    ...
    values.forEach((value: unknown) => {
      ...
      const memoized = memoize(throwValue, 1);
      let firstError;
      let secondError;
...
describe('typing', () => {
  it('should maintain the type of the original function', () => {
    ...
    const memoizedSubtract: SubtractFn = memoize(subtract, 1);

    const result1 = requiresASubtractFn(memoizedSubtract);
    const result2 = requiresASubtractFn(memoize(subtract, 1));
  ...
  it('should support typed equality functions', () => {
    ...
    valid.forEach((isEqual: EqualityFn) => {
      const memoized = memoize(subtract, 1, isEqual);
